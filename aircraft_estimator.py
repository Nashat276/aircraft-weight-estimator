...
.subtitle { font-family:'Share Tech Mono',monospace; color:#4FC3F7; text-align:center; font-size:0.8rem; letter-spacing:0.25em; margin-bottom:1.5rem; }
.metric-card { background:linear-gradient(135deg,#0D1F35,#0A1628); border:1px solid #00D4FF33; border-radius:12px; padding:1.2rem; text-align:center; margin-bottom:0.5rem; }
...
.metric-label { font-family:'Share Tech Mono',monospace; font-size:0.65rem; color:#7B9CC8; letter-spacing:0.12em; margin-top:0.3rem; }
.sec-hdr { font-family:'Orbitron',monospace; font-size:0.85rem; font-weight:700; color:#8A2BE2; letter-spacing:0.18em; border-bottom:1px solid #8A2BE230; padding-bottom:0.35rem; margin-bottom:0.9rem; text-transform:uppercase; }
.status-ok { background:#00FF8820; border:1px solid #00FF8860; border-radius:8px; padding:0.5rem 1rem; font-family:'Share Tech Mono',monospace; font-size:0.85rem; color:#00FF88; }
.status-warn { background:#FF880020; border:1px solid #FF880060; border-radius:8px; padding:0.5rem 1rem; font-family:'Share Tech Mono',monospace; font-size:0.85rem; color:#FF8800; }
[data-testid="stSidebar"] { background:linear-gradient(180deg,#07111F,#0A1628) !important; border-right:1px solid #00D4FF20 !important; }
</style>
""", unsafe_allow_html=True)

# ── layout dict used in every chart ──────────────────────────────────────────
def chart_layout(title='', height=350, extra=None):
    d = dict(
        ...
        xaxis=dict(gridcolor='#00D4FF12', linecolor='#00D4FF30',
                   tickfont=dict(color='#4FC3F7')),
        yaxis=dict(gridcolor='#00D4FF12', linecolor='#00D4FF30',
                   tickfont=dict(color='#4FC3F7')),
    )
    if title:
        d['title'] = dict(text=title, font=dict(color='#4FC3F7', size=13))
    if height:
        d['height'] = height
    if extra:
        d.update(extra)
...
def compute_mission(p):
    ...
    W5_W4 = 1/math.exp(Rc_sm/(375*(p['np_c']/p['Cp_c'])*p['LD_c']))
    Vmph  = p['V_lkts']*1.15078
    W6_W5 = 1/math.exp(p['E_l']/(375*(1/Vmph)*(p['np_l']/p['Cp_l'])*p['LD_l']))

    fracs = dict(zip(
        ['Engine Start','Taxi','Takeoff','Climb','Cruise','Loiter','Descent','Landing'],
        [0.990, 0.995, 0.995, 0.985, W5_W4, W6_W5, 0.985, 0.995]
    ))
    Mff = 1.0
...
def solve_wto(p, tol=1.0, n=200):
    pp = dict(p); lo, hi = 5000.0, 500000.0
    r = {}
    for _ in range(n):
        mid = (lo+hi)/2; pp['Wto_guess'] = mid
...
def sens_calc(p, Wto):
    ...
    B = p['B']
    denom = C*Wto*(1-B)-D
    F = (-B*Wto**2*(1+p['Mres'])*Mff)/denom if abs(denom)>1e-6 else 0
    E = p['E_l']
    return dict(
        dCp_R =  F*Rc_sm/(375*p['np_c']*p['LD_c']),
        dnp_R = -F*Rc_sm*p['Cp_c']/(375*p['np_c']**2*p['LD_c']),
        dLD_R = -F*Rc_sm*p['Cp_c']/(375*p['np_c']*p['LD_c']**2),
        dR    =  F*p['Cp_c']/(375*p['np_c']*p['LD_c']),
        dCp_E =  F*E*Vmph/(375*p['np_l']*p['LD_l']),
        dnp_E = -F*E*Vmph*p['Cp_l']/(375*p['np_l']**2*p['LD_l']),
        dLD_E = -F*E*Vmph*p['Cp_l']/(375*p['np_l']*p['LD_l']**2),
    )


# ── defaults ──────────────────────────────────────────────────────────────────
DEF = dict(n_pax=34,w_pax=175,w_bag=30,n_crew=2,n_att=1,
           Mtfo=0.005,Mres=0.0,range_nm=1100,
           V_ckts=250,LD_c=13,Cp_c=0.60,np_c=0.85,
           E_l=0.75,V_lkts=250,LD_l=16,Cp_l=0.65,np_l=0.77,
           A=0.3774,B=0.9647)
...
with st.sidebar:
    ...
    np_c     = st.slider("η_p cruise",       0.5,0.95, DEF['np_c'],  step=0.01)
    ...
    np_l   = st.slider("η_p loiter",         0.5,0.95, DEF['np_l'],  step=0.01)
...
P = dict(n_pax=n_pax,w_pax=w_pax,w_bag=w_bag,n_crew=n_crew,n_att=n_att,
         ...
         V_ckts=250,LD_c=LD_c,Cp_c=Cp_c,np_c=np_c,
         E_l=E_l,V_lkts=V_lkts,LD_l=LD_l,Cp_l=Cp_l,np_l=np_l,
         A=A_c,B=B_c,Wto_guess=48550)
...
if abs(R['diff']) < 5:
    st.markdown(f'<div class="status-ok">✔ CONVERGED — ΔWE = {R["diff"]:.2f} lbs | W_TO = {Wto:,.0f} lbs</div>', unsafe_allow_html=True)
else:
...
with tab1:
    ...
    for col,val,unit,lbl in [
        ...
        (c2,f"{R['Mff']:.4f}","","FUEL FRACTION M_FF"),
        (c3,f"{R['WF']:,.0f}","lbs","TOTAL FUEL W_F"),
        (c4,f"{R['Wpl']:,.0f}","lbs","PAYLOAD W_PL"),
    ]:
        with col:
    ...
    with ca:
        st.markdown('<div class="sec-hdr">MISSION WEIGHT FRACTIONS</div>', unsafe_allow_html=True)
        phases = list(R['fracs'].keys())
        fracs  = list(R['fracs'].values())
        bar_clr = ['#8A2BE2','#7B6FD4','#6BB4C6','#00D4FF','#00C4EE','#00FF88','#00D070','#00B060']
        fig = go.Figure()
        ...
        fig.update_layout(**chart_layout('Phase Weight Fractions', 340))
        fig.update_yaxes(range=[0.80, 1.02])
        st.plotly_chart(fig, use_container_width=True)

    with cb:
        st.markdown('<div class="sec-hdr">WEIGHT BUDGET GAUGE</div>', unsafe_allow_html=True)
        fig2 = go.Figure(go.Indicator(
            mode="gauge+number+delta",
            value=Wto,
            delta={'reference':DEF.get('Wto_guess',48550),'relative':False,
                   'font':{'color':'#00FF88','size':14}},
            number={'suffix':' lbs','font':{'color':'#00D4FF','size':20,'family':'Share Tech Mono'}},
            gauge={
                'axis':{'range':[10000,150000],'tickfont':{'size':9}},
        ...
        st.markdown('<div class="sec-hdr">WE CONVERGENCE</div>', unsafe_allow_html=True)
        st.dataframe(pd.DataFrame({
            'Parameter':['WE Tentative','WE Allowable','Delta'],
            'Value (lbs)':[f"{R['WE']:,.1f}",f"{R['WE_all']:,.1f}",f"{R['diff']:+.2f}"],
        }), hide_index=True, use_container_width=True)

    st.markdown('<div class="sec-hdr">W_TO vs RANGE SWEEP</div>', unsafe_allow_html=True)
    rng_arr = np.linspace(200, 3000, 55)
    wto_arr = []
    for rv in rng_arr:
        try:
            w,_ = solve_wto({**P,'range_nm':float(rv)})
            wto_arr.append(w)
        except:
            wto_arr.append(float('nan'))
    fig3 = go.Figure()
    ...
    fig3.update_layout(**chart_layout('Gross Takeoff Weight vs Range', 320,
                                       extra=dict(xaxis_title='Range (nm)',
                                                  yaxis_title='W_TO (lbs)')))
...
# ════════════════════════════════════════════════════════════════════════════
with tab2:
    st.markdown('<div class="sec-hdr">BREGUET SENSITIVITY PARTIALS (TABLE 2.20)</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="sec-hdr">TORNADO DIAGRAM</div>', unsafe_allow_html=True)
    t_lbl = ['Cp(range)','ηp(range)','L/D(range)','R(range)',
             'Cp(loiter)','ηp(loiter)','L/D(loiter)']
    t_val = [S['dCp_R'],S['dnp_R'],S['dLD_R'],S['dR']*range_nm*0.1,
             S['dCp_E'],S['dnp_E'],S['dLD_E']]
    order = sorted(range(7), key=lambda i: abs(t_val[i]))
    t_lbl = [t_lbl[i] for i in order]
    t_val = [t_val[i] for i in order]
    fig_t = go.Figure(go.Bar(
        x=t_val, y=t_lbl, orientation='h',
        marker_color=['#00D4FF' if v>=0 else '#FF3CAC' for v in t_val],
        text=[f'{v:+,.0f}' for v in t_val], textposition='outside',
        textfont=dict(size=10),
    ))
    fig_t.add_vline(x=0, line_color='#C8D8F060', line_width=1)
    fig_t.update_layout(**chart_layout('Tornado: ΔW_TO per unit change', 380,
                                        extra=dict(xaxis_title='ΔW_TO (lbs)')))
    st.plotly_chart(fig_t, use_container_width=True)

    st.markdown('<div class="sec-hdr">3D SURFACE — W_TO vs Cp × L/D</div>', unsafe_allow_html=True)
    cp_arr = np.linspace(0.40, 0.90, 20)
    ld_arr = np.linspace(9.0, 20.0, 20)
    Z = np.zeros((len(cp_arr), len(ld_arr)))
    for i,cp in enumerate(cp_arr):
        for j,ld in enumerate(ld_arr):
            try:
                w,_ = solve_wto({**P,'Cp_c':float(cp),'LD_c':float(ld)})
                Z[i,j] = w
            except:
                Z[i,j] = float('nan')
    ...
    fig4.update_layout(
        ...
        title=dict(text='3D: W_TO over Cp and L/D',font=dict(color='#4FC3F7',size=13)),
        scene=dict(
...
with tab3:
    ...
    with cp1:
        st.markdown('<div class="sec-hdr">W_TO BREAKDOWN</div>', unsafe_allow_html=True)
        fig_pie = go.Figure(go.Pie(
            labels=['Empty','Fuel (usable)','Trapped Fuel','Crew','Payload'],
            values=[WE, R['WF_used'], Wtfo, Wcrew, Wpl],
            hole=0.5,
        ...
        fig_pie.update_layout(
            ...
            annotations=[dict(text=f'{Wto:,.0f}<br>lbs',x=0.5,y=0.5,
                               showarrow=False,
                               font=dict(size=13,color='#00D4FF',family='Orbitron'))],
        )
        st.plotly_chart(fig_pie, use_container_width=True)

    with cp2:
        st.markdown('<div class="sec-hdr">WEIGHT THROUGH MISSION</div>', unsafe_allow_html=True)
        fv = list(R['fracs'].values())
        pl = list(R['fracs'].keys()) + ['End']
        cum = [Wto]
        for f in fv:
            cum.append(cum[-1]*f)
        fig_w = go.Figure()
        ...
        fig_w.update_layout(**chart_layout('Weight Through Mission Phases', 320,
                                            extra=dict(xaxis_title='Phase',
    ...
    st.markdown('<div class="sec-hdr">W_TO vs PASSENGER COUNT</div>', unsafe_allow_html=True)
    pax_r = np.arange(5, n_pax+20, 2)
    wto_r = []
    for np_ in pax_r:
        try:
            w,_ = solve_wto({**P,'n_pax':int(np_)})
            wto_r.append(w)
        except:
            wto_r.append(float('nan'))
    fig_pr = go.Figure()
    ...
    fig_pr.update_layout(**chart_layout('W_TO vs Passenger Count', 300,
                                         extra=dict(xaxis_title='Passengers',
                                                    yaxis_title='W_TO (lbs)')))
...
with tab4:
    ...
    with ce1:
        ...
        rows = {
            'Parameter':['W_TO','Mff','W_F','W_payload','W_empty','W_OE','W_crew','W_tfo',
                          'WE_allow','Convergence_delta',
                          'dWTO_dCp_range','dWTO_dnp_range','dWTO_dLD_range','dWTO_dR',
                          'dWTO_dCp_loiter','dWTO_dnp_loiter','dWTO_dLD_loiter'],
            'Value':[Wto,R['Mff'],WF,Wpl,WE,WOE,Wcrew,Wtfo,
                     R['WE_all'],R['diff'],
        ...
        st.download_button("⬇ DOWNLOAD FRACTIONS", buf2.getvalue(),
    ...
    with ce2:
        ...
        def make_pdf():
            ...
            story = [
                Paragraph("AeroNova Simulator X — Mission Report", sT),
                Paragraph("Propeller-Driven Aircraft Preliminary Sizing (Breguet)", sB),
                Spacer(1,0.2*inch),
        ...
        st.download_button("⬇ DOWNLOAD PDF", make_pdf(),
                           "aeronova_report.pdf","application/pdf", use_container_width=True)

    st.markdown("---")
    st.markdown('<div class="sec-hdr">CONFIGURATION SNAPSHOT</div>', unsafe_allow_html=True)

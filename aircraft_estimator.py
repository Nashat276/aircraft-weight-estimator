...
# --- 2. SIDEBAR: THE ENGINEERING COMMAND CENTER ---
# إضافة شعار طائرة ملهم في القائمة الجانبية
st.sidebar.image("https://upload.wikimedia.org/wikipedia/commons/thumb/c/c5/Airplane_silhouette.svg/640px-Airplane_silhouette.svg.png", width=100)
st.sidebar.markdown("### 🛠️ Configuration Dashboard")

with st.sidebar:
    st.markdown("---")
    wto_input = st.number_input("Design WTO (lbs)", value=48550.0, step=10.0, help="Initial Gross Weight for Iteration.")
...
# --- 3. AEROSPACE PHYSICS ENGINE (Table 2.20 Compliant) ---
def run_solver(wto):
...
# إضافة صورة طائرة في رأس الصفحة تعبر عن التنسيق اللوني والاحترافية
st.image("https://images.unsplash.com/photo-1559101168-7c8702b89f81?q=80&w=1600", use_column_width=True)
st.title("✈️ AeroOptimizer Ultra")
st.markdown("##### Conceptual Sizing, Convergence & Sensitivity Analysis | JUST Engineering Standards")
st.divider()
...
with col_map:
    ...
    fig.add_trace(go.Scatter(x=[wto_input], y=[res['we_req']], mode='markers', marker=dict(size=15, color='#3fb950', symbol='diamond'), name='Current Design'))
...
st.write("Quantitative analysis of WTO response to parameter variations.")
...
def generate_pdf(data):
    ...
    pdf.cell(0, 5, "Quantitative Master Sizing Report | Boeing/Airbus Standard", ln=True, align='C')
    ...
    sections = [
        ("1. DESIGN METHODOLOGY", "Balances mission fuel fractions against statistical structural weight modeling to identify the design equilibrium."),
        ("2. CONFIGURATION MATRIX", f"Target WTO: {wto_input:,.1f} lbs\nMission Range: {rc} miles\nL/D Cruise: {ld_c}\nPayload D: {d_val:,.1f} lbs"),
        ("3. CONVERGENCE SUMMARY", f"Mission Fuel Fraction: {data['mff']:.4f}\nGrowth Factor F: {data['f_growth']:.2f}\nStructural Matching Error: {data['we_req'] - data['we_allow']:,.1f} lbs"),
        ("4. SENSITIVITY DERIVATIVES (dW/dX)", f"Range Penalty (dW/dR): {data['dw_dr']:.4f} lbs/mi\nSFC Penalty (dW/dCp): {data['dw_dcp']:.2f} lbs/unit SFC")
    ]
...
st.download_button("📥 DOWNLOAD MASTER DESIGN PACKAGE (PDF)", 
                   ...
                   file_name="Aero_Master_Package.pdf")

...
.main-title{font-family:'Orbitron',monospace;font-size:2.2rem;font-weight:900;
  ...
  animation:shimmer 3s infinite;text-align:center;letter-spacing:.12em}
@keyframes shimmer{0%{background-position:0%}100%{background-position:200%}}
.subtitle{font-family:'Share Tech Mono',monospace;color:#4FC3F7;text-align:center;
  font-size:.8rem;letter-spacing:.25em;margin-bottom:1.5rem}
...
.metric-unit{font-size:.85rem;color:#4FC3F7}
.metric-label{font-family:'Share Tech Mono',monospace;font-size:.65rem;color:#7B9CC8;
  letter-spacing:.12em;margin-top:.3rem}
.sec-hdr{font-family:'Orbitron',monospace;font-size:.85rem;font-weight:700;color:#8A2BE2;
  letter-spacing:.18em;border-bottom:1px solid #8A2BE230;padding-bottom:.35rem;
  margin-bottom:.9rem;text-transform:uppercase}
.status-ok{background:#00FF8820;border:1px solid #00FF8860;border-radius:8px;
  padding:.5rem 1rem;font-family:'Share Tech Mono',monospace;font-size:.85rem;color:#00FF88}
.status-warn{background:#FF880020;border:1px solid #FF880060;border-radius:8px;
  padding:.5rem 1rem;font-family:'Share Tech Mono',monospace;font-size:.85rem;color:#FF8800}
[data-testid="stSidebar"]{background:linear-gradient(180deg,#07111F,#0A1628)!important}
</style>
""", unsafe_allow_html=True)


# ── safe layout: NO xaxis/yaxis keys — use update_xaxes/update_yaxes instead ─
PAPER = dict(
...
def set_axes(fig, xt='', yt='', yrange=None):
    fig.update_xaxes(gridcolor='#00D4FF12', linecolor='#00D4FF30')
    fig.update_yaxes(gridcolor='#00D4FF12', linecolor='#00D4FF30')
    if xt:     fig.update_xaxes(title_text=xt)
    if yt:     fig.update_yaxes(title_text=yt)
    if yrange: fig.update_yaxes(range=yrange)

def styled(fig, title='', height=350, xt='', yt='', yrange=None):
    kw = dict(PAPER)
    if title:  kw['title']  = dict(text=title, font=dict(color='#4FC3F7', size=13))
    if height: kw['height'] = height
    fig.update_layout(**kw)
    set_axes(fig, xt=xt, yt=yt, yrange=yrange)
...
with st.sidebar:
    ...
    V_lkts=st.slider("Loiter spd(kts)",100, 350,DEF['V_lkts'])
...
with tab1:
    ...
    with ca:
        ...
        styled(fig,'Phase Weight Fractions',340,yrange=[0.80,1.02])
    ...
    for rv in rng_arr:
        try: w,_=solve_wto({**P,'range_nm':float(rv)});wto_arr.append(w)
    ...
    styled(fig3,'W_TO vs Range',320,xt='Range (nm)',yt='W_TO (lbs)')
...
with tab2:
    ...
    styled(fig_t,'Tornado: dW_TO per unit change',380,xt='dW_TO (lbs)')
    ...
    for i,cp in enumerate(cp_arr):
        for j,ld in enumerate(ld_arr):
            try: w,_=solve_wto({**P,'Cp_c':float(cp),'LD_c':float(ld)});Z[i,j]=w
...
# ── TAB 3 ─────────────────────────────────────────────────────────────────────
with tab3:
    WE=R['WE'];WOE=R['WOE'];WF=R['WF'];Wpl=R['Wpl'];Wcrew=R['Wcrew'];Wtfo=R['Wtfo']
    ...
    with cp2:
        ...
        styled(fig_w,'Weight Through Mission',320,xt='Phase',yt='Weight (lbs)')
    ...
    for np_ in pax_r:
        try: w,_=solve_wto({**P,'n_pax':int(np_)});wto_r.append(w)
    ...
    styled(fig_pr,'W_TO vs Passengers',300,xt='Passengers',yt='W_TO (lbs)')
